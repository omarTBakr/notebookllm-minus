...
async def drain_ingestion(app):
    ...
    # Processing first, then indexing — the order the chain enforces, and the
    # order that matters: indexing a project whose chunks do not exist yet
    # raises rather than silently indexing nothing.
    for task in sorted(pending, key=lambda t: not t.task_name.endswith("process_data_task")):
        args = task.args
        project_id = args["project_id"]

        if task.task_name.endswith("process_data_task"):
