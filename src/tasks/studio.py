...
def generate_artifact_task(self, chat_id: str, kind: str) -> dict:
    ...
    try:
        ...
        # The nested finally in _run_generation closes the provider pools and
        # then the DB. A hard kill skips both, leaking an HTTP connection pool
        # per timed-out run.
        logger.error(
            "generate_artifact_task for %r exceeded its soft time limit of %ss",
            chat_id,
            SETTINGS.CELERY_TASK_SOFT_TIME_LIMIT,
        )
        raise CeleryTaskError(
            f"Generating {kind} for {chat_id!r} exceeded " f"{SETTINGS.CELERY_TASK_SOFT_TIME_LIMIT}s and was stopped"
        ) from exc
